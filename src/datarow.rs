...
impl DataRow {
    ...
    pub fn try_create(row: &str) -> Result<DataRow, DataRowError> {
        ...
        fields.push(tfr(row, "CyclNo1", 11, 16, &datafield::trim)?);
        fields.push(tfr(row, "Status", 16, 23, &datafield::trim)?);
        fields.push(tfr(row, "OwnerName", 23, 49, &datafield::trim)?);
        fields.push(tfr(row, "PropAddrKey", 49, 60, &datafield::trim)?);
        fields.push(tfr(row, "AddrLine2", 60, 86, &datafield::trim)?);
        fields.push(tfr(row, "MeterID", 86, 100, &datafield::validate_acct)?);
        fields.push(tfr(row, "CyclNo2", 100, 105, &datafield::trim)?);
        fields.push(tfr(row, "ReadDigits", 105, 110, &datafield::trim)?);
        fields.push(tfr(row, "No", 110, 114, &datafield::trim)?);
        fields.push(tfr(row, "Type", 114, 119, &datafield::trim)?);
        fields.push(tfr(row, "ARB", 119, 130, &datafield::trim)?);
        fields.push(tfr(row, "FileKey", 130, 141, &datafield::trim)?);
        fields.push(tfr(row, "StreetDirection", 141, 151, &datafield::trim)?);
        fields.push(tfr(row, "StreetName", 151, 177, &datafield::trim)?);
        fields.push(tfr(row, "StreetNumber", 177, 184, &datafield::trim_zeroes)?);
        fields.push(tfr(row, "StreetUnit", 184, 191, &datafield::trim)?);
        fields.push(tfr(row, "MeterSerial", 191, 205, &datafield::trim)?);
